...
def run_micropython(pyb, args, test_file, is_special=False):
    special_tests = (
        ...
        'basics/builtin_help.py', 'basics/exception_chain.py',
        'thread/thread_exc2.py', 'esp32/partition_ota.py',
    )
    had_crash = False
    if pyb is None:
        # run on PC
...
''')
    ...
    cmd_parser.add_argument('--mpy-cross-flags', default='-mcache-lookup-bc', help='flags to pass to pycopy-cross')
    cmd_parser.add_argument('--keep-path', action='store_true', help='do not clear PYCOPYPATH when running tests')
    ...
    if len(args.files) == 0:
        if args.test_dirs is None:
            ...
            if args.target == 'pyboard':
                ...
                # run PC tests
                test_dirs += ('strict', 'float', 'import', 'io', 'stress', 'unicode', 'unix', 'cmdline', 'bytecode')
            elif args.target == 'qemu-arm':
