...
.. class:: AbstractStream
    ...
    .. method:: read(size=-1)
        ...
        Read *size* bytes or characters of the stream, or until EOF if
    ...
    .. method:: readline(maxsize=-1)
        ...
        until ``'\n'`` character is seen. The line, with ``'\n'`` included,
        is returned. If positional *maxsize* argument is provided, at
        most *maxsize* bytes are read (in which case the returned
        string might not end with ``'\n'``).

    .. method:: readinto(buf, [maxsize])

        Read bytes into a *buf* `buffer`. The size of data read is equal
        to the size of buffer, unless *maxsize* is specified, which allows
        to read less data. Returns number of bytes actually read. This
        method is available only for binary streams.
    ...
    .. method:: write(data)
        ...
        or arbitrary `buffer` for binary streams.
